...
import { Save, Eye, EyeOff, Lock, User, ImagePlus, Trash2 } from 'lucide-react'
...
export default function ConfiguracoesPage() {
  ...
  const inputSt: React.CSSProperties = {
    height: '40px', padding: '0 12px', borderRadius: 10, fontSize: 13,
  ...
  const cardBodySt: React.CSSProperties = {
    padding: '22px 22px 6px', display: 'flex', flexDirection: 'column', gap: 14,
  }
  const cardFootSt: React.CSSProperties = {
    display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12,
    padding: '14px 22px 20px', background: 'var(--paper)', borderTop: '1px solid var(--line)',
  }
  ...
  function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    rd.onload = ev => setProfile({ ...profile, photo: ev.target?.result as string })
    rd.readAsDataURL(f)
  }
    ...
    <div style={{ background: 'var(--paper)', minHeight: '100%' }}>
      ...
      <div style={{ padding: '24px 32px 18px' }}>
        ...
        <h1 style={{
          ...
          letterSpacing: '-0.03em', lineHeight: 1.1, color: 'var(--ink)',
        }}>
          Configurações
        </h1>
      ...
      <div style={{ padding: '6px 32px 40px', display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 560 }}>

        {/* Perfil */}
        <div style={cardSt}>
          <div style={cardHeadSt}>
            <User size={15} strokeWidth={1.75} color="var(--ink-2)" />
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>Perfil</span>
          </div>
          <div style={cardBodySt}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 18, paddingBottom: 6 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                >
                  <ImagePlus size={13} /> {profile.photo ? 'Trocar foto' : 'Adicionar foto'}
                </button>
                {profile.photo && (
                  <button
                ...
                <span style={{ fontSize: 11.5, color: 'var(--muted)', lineHeight: 1.5 }}>
                  {profile.photo ? 'Aparece no menu lateral.' : 'Opcional — sem foto, exibe a inicial do nome.'}
                </span>
              </div>
          ...
          <div style={cardFootSt}>
            {savedDados && (
              <span style={{ fontSize: 12.5, color: 'var(--success)', fontWeight: 600 }}>Salvo!</span>
            )}
            <button onClick={saveDados} style={{
              display: 'flex', alignItems: 'center', gap: 6,
        ...
        <div style={cardSt}>
          <div style={cardHeadSt}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>Segurança</span>
          </div>
          <div style={cardBodySt}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={labelSt}>Senha atual</label>
              <PasswordInput value={senhaAtual} onChange={setSenhaAtual} show={showAtual} onToggle={() => setShowAtual(v => !v)} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={labelSt}>Nova senha</label>
              <PasswordInput value={novaSenha} onChange={setNovaSenha} show={showNova} onToggle={() => setShowNova(v => !v)} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={labelSt}>Confirmar nova senha</label>
              <PasswordInput value={confirmarSenha} onChange={setConfirmarSenha} show={showConfirmar} onToggle={() => setShowConfirmar(v => !v)} />
            </div>
          </div>
          <div style={cardFootSt}>
            {senhaErro && (
              <span style={{ fontSize: 12.5, color: 'var(--brand)', fontWeight: 600 }}>{senhaErro}</span>
            )}
            {savedSenha && (
              <span style={{ fontSize: 12.5, color: 'var(--success)', fontWeight: 600 }}>Senha alterada!</span>
            )}
            <button onClick={saveSenha} style={{
              display: 'flex', alignItems: 'center', gap: 6,
              padding: '8px 18px', borderRadius: 10, border: 'none',
