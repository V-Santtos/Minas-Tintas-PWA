...
import {
  Home, ShoppingBag, Users, Gift, BarChart2,
  LogOut, Settings,
} from 'lucide-react'
...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--paper-deep)' }}>
      ...
      <div style={{
        ...
        >
          ...
          <div style={{
            ...
            {isExpanded
              ? <Image src="/logo.png" alt="Minas Tintas" width={140} height={52} style={{ height: 52, width: 'auto' }} />
              : <Image src="/assets/logo-m.png" alt="M" width={509} height={251} style={{ height: 28, width: 'auto', maxWidth: '100%' }} />
            }
          </div>
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {NAV.map(({ href, icon: Icon, label, badge }) => {
              ...
              return (
                ...
                >
                  <Icon size={18} strokeWidth={1.75} style={{ flexShrink: 0, color: active ? '#fff' : 'rgba(255,255,255,.5)' }} />
                  {isExpanded && (
                    <span style={{ overflow: 'hidden', whiteSpace: 'nowrap', flex: 1 }}>
                      {label}
                    </span>
                  )}
                  {isExpanded && badge && (
                    <span style={{
                      marginLeft: 'auto', background: '#fff', color: 'var(--brand)',
                      fontSize: 10, fontWeight: 700, padding: '2px 7px',
                      borderRadius: 999, lineHeight: 1.4, flexShrink: 0,
                    }}>
                      {badge}
                    </span>
                  )}
          ...
          >
            ...
            {isExpanded && (
              <div style={{ overflow: 'hidden', whiteSpace: 'nowrap', flex: 1 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: '#fff' }}>{profile.name}</div>
