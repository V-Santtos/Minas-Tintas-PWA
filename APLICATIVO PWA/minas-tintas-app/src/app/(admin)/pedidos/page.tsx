...
import { Search, SlidersHorizontal, Plus, ChevronRight, ArrowDown, ArrowUp, ChevronsUpDown, Clock, CheckCircle2, Gift, TrendingUp, X, HardHat, Building2, User, Package, Minus, Banknote, QrCode, CreditCard, Receipt, UserPlus } from 'lucide-react'
import { CATALOG, CLIENTS, ORDERS, PAINTERS, brl, bonus, type Order, type Client } from '@/lib/mock'
import { CadastrarClienteModal, loadManualClients } from '@/components/CadastrarClienteModal'
...
const STATUS_CFG: Record<string, { bg: string; fg: string; border: string; dot: string; weight: number }> = {
  pendente:  { bg: '#FFF0DC', fg: '#C86408', border: '#F3C884', dot: '#E07A10', weight: 700 },
  aprovado:  { bg: '#EDF3E9', fg: '#4F7A4A', border: '#DCE7D5', dot: '#6D9168', weight: 600 },
  recusado:  { bg: '#FBEAEA', fg: '#A30000', border: '#F2CFCF', dot: '#CC0000', weight: 650 },
  estornado: { bg: '#F2EDE4', fg: '#766D66', border: '#E2D9CC', dot: '#8A817A', weight: 600 },
}
...
function calcStats(orders: Order[]) {
  ...
  const todayApproved = orders.filter(o => o.date === TODAY && o.status === 'aprovado')
  ...
  return {
    ...
    bonusTotal:   approved.reduce((acc, o) => acc + bonus(o.total), 0),
...
export default function PedidosPage() {
  ...
  const painterOptions = Array.from(new Set(orders.map(o => o.painter))).sort()
  const activeAdvancedFilters = [
    painterFilter,
    paymentFilter !== 'todos',
  ].filter(Boolean).length
  const painterMatches = manualDraft.painter.trim()
    ? PAINTERS.filter(p => p.active && p.name.toLowerCase().includes(manualDraft.painter.toLowerCase().trim())).slice(0, 5)
    : []
  const titleQuery = manualDraft.title.toLowerCase().trim()
  const clientMatches = titleQuery
    ? allClients.filter(c => c.name.toLowerCase().includes(titleQuery)).slice(0, 6)
    : []
  const showNewClientTrigger = titleQuery && clientMatches.length === 0
  const productMatches = manualDraft.productSearch.trim()
    ? CATALOG.filter(p =>
        p.name.toLowerCase().includes(manualDraft.productSearch.toLowerCase().trim()) ||
        p.brand.toLowerCase().includes(manualDraft.productSearch.toLowerCase().trim()) ||
        p.code.toLowerCase().includes(manualDraft.productSearch.toLowerCase().trim())
      ).slice(0, 5)
    : []
  const cartEntries = Object.entries(manualCart)
    .map(([id, qty]) => ({ product: CATALOG.find(p => p.id === id), qty }))
    .filter((entry): entry is { product: typeof CATALOG[number]; qty: number } => Boolean(entry.product) && entry.qty > 0)
  const manualTotal = cartEntries.reduce((sum, { product, qty }) => sum + product.price * qty, 0)

  const q = search.toLowerCase()
  let filtered = statusFilter === 'todos' ? [...orders] : orders.filter(o => o.status === statusFilter)
  ...
  if (painterFilter) filtered = filtered.filter(o => o.painter === painterFilter)
  if (paymentFilter !== 'todos') filtered = filtered.filter(o => paymentGroup(o.payment) === paymentFilter)
  ...
  const filterCount = (key: Status) =>
    key === 'todos' ? orders.length : orders.filter(o => o.status === key).length
  ...
  function submitManualOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const painter = PAINTERS.find(p => p.active && p.name === manualDraft.painter.trim())
    if (!manualDraft.title.trim() || cartEntries.length === 0 || !manualDraft.payment) return

    const nextNumber = Math.max(...orders.map(o => Number(o.id)).filter(Number.isFinite), 0) + 1
    ...
    const order: Order = {
      ...
      items: cartEntries.map(({ product, qty }) => ({ name: product.name, qty, unit: product.price })),
      status: 'pendente',
      payment: manualDraft.payment,
      notes: manualDraft.notes.trim(),
    }

    const currentManual = orders.filter(o => Number(o.id) > Math.max(...ORDERS.map(x => Number(x.id))))
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 28, letterSpacing: '-0.03em', color: 'var(--ink)', lineHeight: 1.05 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            borderRadius: 10, padding: '0 13px', height: 38,
            fontSize: 13, color: searchFocused ? 'var(--brand)' : 'var(--muted)',
            width: 320,
      ...
      {filtersOpen && (
        <div style={{ padding: '14px 32px 0' }}>
          <div style={{
            ...
            <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
              Pintor
              <select value={painterFilter} onChange={e => setPainterFilter(e.target.value)} style={{
                height: 38, borderRadius: 10, border: '1px solid var(--line)',
                background: 'var(--card)', padding: '0 10px', fontFamily: 'var(--font-body)',
                color: 'var(--ink)', outline: 'none',
              }}>
                <option value="">Todos os pintores</option>
                {painterOptions.map(painter => <option key={painter} value={painter}>{painter}</option>)}
              </select>
            </label>
            <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
              Forma de pagamento
      ...
      <div style={{
        ...
        gap: 14, padding: '20px 32px 20px',
      }}>
        {/* Pendentes */}
        <div style={{
          background: 'var(--card)', border: '1px solid var(--line)',
          borderRadius: 14, display: 'flex', overflow: 'hidden',
          boxShadow: '0 2px 6px rgba(28,26,23,.06), 0 1px 2px rgba(28,26,23,.04)',
        }}>
          <div style={{ width: 52, flexShrink: 0, background: '#E07A10', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Clock size={22} strokeWidth={1.75} style={{ color: '#fff' }} />
          </div>
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 5 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--muted)' }}>PENDENTES</div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 30, color: '#E07A10', letterSpacing: '-0.03em', lineHeight: 1 }}>
              {stats.pendingCount}
            </div>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>aguardando pagamento</div>
          </div>
        </div>
        {/* Hoje aprovado */}
        <div style={{
          ...
          <div style={{ width: 52, flexShrink: 0, background: '#4F7A4A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <CheckCircle2 size={22} strokeWidth={1.75} style={{ color: '#fff' }} />
          </div>
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 5 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--muted)' }}>HOJE · {TODAY.toUpperCase()}</div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 30, color: '#4F7A4A', letterSpacing: '-0.03em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
              R$ {stats.todayTotal.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}
            </div>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>
              {stats.todayCount} pedido{stats.todayCount !== 1 ? 's' : ''} aprovado{stats.todayCount !== 1 ? 's' : ''}
            </div>
          </div>
        </div>
        {/* Bônus */}
        <div style={{
          ...
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 5 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--muted)' }}>BÔNUS LIBERADO · TOTAL</div>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>pts creditados aos pintores</div>
          </div>
        </div>
        {/* Volume do dia */}
        <div style={{
          ...
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 5 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--muted)' }}>HOJE · VOLUME</div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 30, color: 'var(--ink)', letterSpacing: '-0.03em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
              R$ {stats.todayVolume.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}
            </div>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>
              {stats.todayAllCount} pedido{stats.todayAllCount !== 1 ? 's' : ''} no dia
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '0 32px 32px' }}>
            ...
            {FILTERS.map(f => (
              <button
                ...
                style={{
                  ...
                  background: statusFilter === f.key ? 'var(--ink)' : 'var(--card)',
                  color: statusFilter === f.key ? 'var(--paper)' : 'var(--ink-2)',
                  border: `1px solid ${statusFilter === f.key ? 'var(--ink)' : 'var(--line)'}`,
                  cursor: 'pointer', transition: 'background .12s, color .12s',
                }}
              >
                {f.dot && (
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: statusFilter === f.key ? 'var(--paper)' : f.dot, flexShrink: 0, display: 'inline-block' }} />
                )}
                {f.label}
                <span style={{ opacity: .6, fontWeight: 500 }}>{filterCount(f.key)}</span>
              </button>
            ))}
          </div>
          <div style={{
            fontSize: 12, color: 'rgba(138,129,122,.82)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: '#F7F2EA', borderBottom: '1px solid var(--line)',
            ...
            >
              Data
              {dateSort === 'desc' ? <ArrowDown size={11} strokeWidth={2} /> : dateSort === 'asc' ? <ArrowUp size={11} strokeWidth={2} /> : <ChevronsUpDown size={11} strokeWidth={2} />}
            </div>
            ...
            return (
              <div
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '70px 1fr 1fr 90px 130px 185px 100px',
                  padding: '14px 18px 14px 15px', gap: 16, alignItems: 'center',
                  borderBottom: '1px solid var(--line)',
                  borderLeft: `3px solid ${isPending ? '#E07A10' : 'transparent'}`,
                  cursor: 'pointer', fontSize: 13, color: 'var(--ink-2)',
                  background: isHovered
                    ? (isPending ? '#FFF7ED' : 'var(--paper-deep)')
                    : (isPending ? 'rgba(253,235,208,.18)' : 'transparent'),
                  transition: 'background .14s, border-color .14s',
                }}
                onMouseEnter={() => setHoveredOrderId(o.id)}
                onMouseLeave={() => setHoveredOrderId(null)}
              >
                <div style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--muted)' }}>{o.id}</div>
                <div>
                  <div style={{ fontWeight: 600, color: 'var(--ink)' }}>{o.painter}</div>
                  <div style={{ fontSize: 11.5, color: 'var(--muted)', marginTop: 2 }}>{o.location}</div>
                </div>
                <div style={{ color: 'var(--ink-2)' }}>{o.title}</div>
                <div style={{ color: 'var(--muted)', fontSize: 12.5 }}>{o.date}</div>
                <div style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                  <strong style={{ color: 'var(--ink)' }}>R$ {brl(o.total)}</strong>
                </div>
                <div>
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', gap: 6,
                    padding: o.status === 'pendente' ? '4px 10px' : '3px 9px',
                    borderRadius: 999,
                    fontSize: 11, fontWeight: s.weight, whiteSpace: 'nowrap',
                    background: s.bg, color: s.fg,
                    border: `1px solid ${s.border}`,
                    boxShadow: o.status === 'pendente' ? '0 1px 2px rgba(224,122,16,.08)' : 'none',
                  }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <button
                    style={{
                      ...
                      cursor: 'pointer',
                      opacity: isHovered ? 1 : .72,
                      transform: isHovered ? 'translateX(0)' : 'translateX(2px)',
        ...
        >
          ...
          >
            <div style={{
              ...
              <div>
                ...
                <p style={{ marginTop: 6, fontSize: 13, color: 'var(--muted)', lineHeight: 1.45 }}>
                  Informe cliente, produtos e pagamento para registrar o pedido manual.
                </p>
              </div>
              <button
                type="button"
            ...
            <div style={{ padding: 22, display: 'grid', gap: 14 }}>
              <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  />
                  {((clientMatches.length > 0 && manualDraft.title !== clientMatches[0]?.name) || showNewClientTrigger) && (
                    <div style={{
                      position: 'absolute', zIndex: 4, left: 0, right: 0, top: 46,
                      background: 'var(--card)', border: '1px solid var(--line)',
                      borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--shadow-3)',
                    }}>
                      {clientMatches.map(c => {
                        ...
                        return (
                          <button
                            key={c.name}
                            type="button"
                            onClick={() => setManualDraft(d => ({ ...d, title: c.name }))}
                            style={{
                          ...
                          >
                            <Icon size={15} strokeWidth={1.75} color="var(--muted)" />
                            <span style={{ flex: 1 }}>
                              <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{c.name}</span>
                              <span style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)' }}>{c.type === 'empresa' ? 'Empresa' : 'Pessoa física'}</span>
                            </span>
                          </button>
                        )
                      })}
                      {showNewClientTrigger && (
                        <button
                          type="button"
                          onClick={() => { setClientModalPrefill(manualDraft.title); setClientModalOpen(true) }}
                          style={{
                            width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                            padding: '10px 12px', border: 'none', background: 'var(--card)',
                        ...
                        >
                          <UserPlus size={15} strokeWidth={1.75} color="var(--brand)" />
                          <span style={{ flex: 1 }}>
                            <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--brand)' }}>
              ...
              <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    onChange={e => setManualDraft(d => ({ ...d, painter: e.target.value }))}
                  ...
                  />
                  {painterMatches.length > 0 && manualDraft.painter !== painterMatches[0]?.name && (
                    <div style={{
                      position: 'absolute', zIndex: 5, left: 0, right: 0, top: 46,
                      background: 'var(--card)', border: '1px solid var(--line)',
                      borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--shadow-3)',
                    }}>
                      {painterMatches.map(p => (
                        <button
                          key={p.name}
                          type="button"
                          onClick={() => setManualDraft(d => ({ ...d, painter: p.name }))}
                          style={{
                            width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                            padding: '10px 12px', border: 'none', background: 'var(--card)',
                            borderBottom: '1px solid var(--line)', textAlign: 'left',
                        ...
                        >
                          <HardHat size={16} strokeWidth={1.75} color="var(--muted)" />
                          <span style={{ flex: 1 }}>
              ...
              <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    onChange={e => setManualDraft(d => ({ ...d, productSearch: e.target.value }))}
                  ...
                  {productMatches.length > 0 && (
                    <div style={{
                      position: 'absolute', zIndex: 3, left: 0, right: 0, top: 46,
                      background: 'var(--card)', border: '1px solid var(--line)',
                      borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--shadow-3)',
                    }}>
                      {productMatches.map(p => {
                        ...
                        return (
                          ...
                          >
                            <Package size={15} strokeWidth={1.75} color="var(--muted)" />
                            <span style={{ flex: 1, minWidth: 0 }}>
                              <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{p.name}</span>
                              <span style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 11.5, color: 'var(--muted)' }}>
                                <span>{p.brand}</span>
                                <span>R$ {brl(p.price)}</span>
                                <span style={{ color: margin >= 30 ? 'var(--success)' : margin >= 15 ? 'var(--warning)' : 'var(--brand)', fontWeight: 600 }}>Margem {margin}%</span>
                                <span style={{ color: stockWarn ? 'var(--brand)' : 'var(--muted)', fontWeight: stockWarn ? 600 : 400 }}>{p.stock === 0 ? 'sem estoque' : `${p.stock} un.`}</span>
                              </span>
                            </span>
                          </button>
                        )
                  ...
                  <div style={{ border: '1px solid var(--line)', borderRadius: 10, overflow: 'hidden' }}>
                    {cartEntries.map(({ product, qty }) => (
                      <div key={product.id} style={{
                        ...
                        <Package size={14} strokeWidth={1.75} color="var(--muted)" />
                        <span style={{ flex: 1, fontSize: 12, color: 'var(--ink)', lineHeight: 1.35 }}>{product.name}</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <button type="button" onClick={() => changeManualProductQty(product.id, -1)} style={{ width: 24, height: 24, borderRadius: 7, border: '1px solid var(--line)', background: 'var(--card)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                          ...
                          <span style={{ minWidth: 78, textAlign: 'right', fontSize: 12, fontWeight: 700, color: 'var(--ink)', fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    {/* Desconto + resumo */}
                    <div style={{ background: 'var(--paper-deep)', borderTop: '1px solid var(--line)' }}>
                      {/* Linha de desconto */}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', borderBottom: '1px solid var(--line)', gap: 8 }}>
                        <span style={{ fontSize: 12, color: 'var(--muted)', whiteSpace: 'nowrap' }}>Desconto para o cliente</span>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                          ...
                          <input
                            ...
                            onChange={e => setManualDraft(d => ({ ...d, discount: e.target.value.replace(/\D/g, '') }))}
                            placeholder="0,00"
                            style={{
                        ...
                        return (
                          <div style={{ padding: '9px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13 }}>
                              <span style={{ color: 'var(--muted)' }}>Total cobrado</span>
                              <strong style={{ fontVariantNumeric: 'tabular-nums' }}>R$ {brl(clientTotal)}</strong>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11.5 }}>
                              <span style={{ color: 'var(--muted)' }}>Bônus do pintor {disc > 0 ? '(sobre subtotal)' : ''}</span>
                              <span style={{ color: 'var(--success)', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                                {bonus(manualTotal).toLocaleString('pt-BR')} pts
                              </span>
                            </div>
                          </div>
                        )
              ...
              <label style={{ display: 'grid', gap: 8, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
                Pagamento informado
                  ...
                  {[
                    ...
                    return (
                      <button
                        key={label}
                        type="button"
                        onClick={() => setManualDraft(d => ({ ...d, payment: label }))}
                        style={{
              ...
              <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
                ...
                <textarea
                  ...
                  style={{
                    ...
                    background: 'var(--card)', padding: 12, resize: 'vertical',
                    fontFamily: 'var(--font-body)', color: 'var(--ink)', outline: 'none',
                  }}
                />
              </label>
            ...
            <div style={{
              ...
              <button type="button" onClick={closeManualModal} style={{
                height: 38, padding: '0 14px', borderRadius: 10,
                border: '1px solid var(--line)', background: 'var(--card)',
