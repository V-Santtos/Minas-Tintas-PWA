...
/* ── data (year-aware) a partir do createdAtISO 'YYYY-MM-DD' ── */
function ymd(o: Order) {
  const iso = o.createdAtISO ?? "";
  return {
    year: +iso.slice(0, 4),
    monthIdx: +iso.slice(5, 7) - 1,
    day: +iso.slice(8, 10),
  };
}
...
export default function RelatoriosClient({
  ...
  orders: Order[];
  painters: { name: string; city: string }[];
}) {
  // defaults: mês/ano do pedido mais recente
  const _dated = orders.map(ymd).filter((d) => d.year > 0);
  const _latest = _dated.reduce(
    (a, b) => (b.year * 12 + b.monthIdx > a.year * 12 + a.monthIdx ? b : a),
    _dated[0] ?? {
  ...
  const [month, setMonth] = useState(_latest.monthIdx);
  const [year, setYear] = useState(_latest.year);
  ...
  if (view === "mes") {
    ...
    const bp: ChartPoint[] = [];
    const yearMonths = [
      ...new Set(periodOrders.map((o) => ymd(o).monthIdx)),
    ].sort((a, b) => a - b);
    yearMonths.forEach((m, mi) => {
      const mApproved = approved.filter((o) => ymd(o).monthIdx === m);
      const mVol = mApproved.reduce((s, o) => s + o.total, 0);
      const mBonus = mApproved.reduce((s, o) => s + (o.bonusPts ?? 0), 0);
    ...
    volData = vp;
    bonusData = bp;
    xDomain = [0, Math.max(3, yearMonths.length * 4 - 1)];
    xTicks = yearMonths.map((_, i) => i * 4);
    xFmt = (v) => MONTH_FULL[yearMonths[Math.floor(v / 4)]]?.slice(0, 3) ?? "";
    xLabelFmt = (v) => MONTH_FULL[yearMonths[Math.floor(v / 4)]] ?? "";
  }

  /* ── ranking ── */
  const ranking = painters
    .map((p) => {
      ...
      return {
        ...
        bonus: pOrds.reduce((s, o) => s + (o.bonusPts ?? 0), 0),
      };
    })
  ...
  /* ── labels ── */
  const periodLabel =
    view === "mes" ? `${MONTH_FULL[month]} ${year}` : String(year);
  ...
  return (
    <div style={{ padding: "32px 32px 40px" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "var(--muted)" }}>
              {sc.aprovado} aprovados · {sc.pendente} pendentes
            </div>
          </div>
        </div>
              ...
              >
                {donutData.map((s) => (
                  ...
                  >
                    ...
                    />
                    <span
                      style={{ fontSize: 12, color: "var(--ink-2)", flex: 1 }}
                    >
                      {s.name}
                    </span>
                    <span
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        {/* Ranking de pintores */}
        <div style={{ ...card, gap: 0, padding: 20 }}>
          <span style={{ ...lbl, marginBottom: 14, display: "block" }}>
            Ranking de pintores
          </span>
