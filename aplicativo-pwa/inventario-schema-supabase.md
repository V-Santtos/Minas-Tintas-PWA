...
| `conceder_brinde_boas_vindas(painter)`       | **service_role** | concede o brinde de boas-vindas: **idempotente** (se o pintor já tem resgate de brinde, retorna o existente); `FOR UPDATE` no boné; se ainda há boné (`stock > 0`) sorteia boné e baixa 1, senão pincel; cria `resgates` grátis (`pontos_congelados = 0`, `iniciado_por = admin`); **sem ledger**. Chamada pelo `POST /api/pintores` após criar o pintor. **Sem `current_painter_id`** — roda em contexto service (sem JWT de usuário); o gate de admin fica no route handler |
