...
| "Em promoção desde quando?"      | `loja_items.promo_desde` (carimbado por trigger na transição)          |
...
erDiagram
    ...
    PRODUCTS {
        ...
        bool active
        uuid source_id "id no Hiper (upsert); nulavel"
    }
    ORDERS {
        uuid id PK
...
- `notif_pedidos` / `notif_pontos` / `notif_resgates` / `notif_promocoes` `boolean` (defaults `true`/`true`/`true`/`false`). **Consumidas** pelo feed de notificações do pintor (T6a): cada tipo de card só entra se a preferência correspondente estiver ligada.
- `brinde_visto_em` `timestamptz` nulável — carimbo de quando o pintor fechou o modal de brinde de boas-vindas (`null` = ainda não viu). Escrito pelo RPC `marcar_brinde_visto`. É o "já viu?" que decide se o modal reabre — antes era stub de `localStorage`.
- `notif_visto_em` `timestamptz` nulável — marco "vi as notificações até este instante"; escrito pelo RPC `marcar_notif_visto` ao abrir a tela. Controla a **bolinha do sininho**: não-lido = existe evento (do feed derivado) com `created_at` mais novo que este valor (`null` = nunca abriu → tudo conta como não lido).

**`push_subscriptions`** — Web Push (T6c): subscriptions de notificação do pintor, **1 linha por
dispositivo/navegador** (`endpoint` text é a PK = identidade do aparelho; o mesmo pintor pode ter N).

- `painter_id` → painters, `on delete cascade` (some o pintor, somem as subscriptions); índice
  por pintor — o **envio** lê por pintor.
- `p256dh` / `auth` `not null` — chaves da subscription (padrão Web Push).
- RLS **self completa** (select/insert/update/delete escopados por
  `painter_id = (select id from painters where auth_user_id = auth.uid())`); o envio roda no
  admin com `service_role` (fura RLS) — admin não precisa de policy.
...
**`products`** — catálogo de venda (orçamentos). Alimentado pela **sync do ERP Hiper**
(upsert por `source_id`); módulo **separado** da lojinha de pontos.

- `source_id` (uuid, **único**, nulável) — `id` do produto no Hiper; chave de upsert
  idempotente. `null` = origem não-Hiper (legado/manual). PK `id` continua local.
- Saída do catálogo no Hiper → `active = false` (**soft-delete**, nunca `delete`): preserva
  o FK/snapshot de `order_items`.
...
- `stock` **nulável** `check (stock >= 0)`: número = estoque finito; `null` = **ilimitado**
  (o check só reprova valores negativos, `null` passa). Usado pelos brindes (o boné tem 10 fixos;
  o pincel é ilimitado).
...
- `is_brinde` `boolean` default `false`: item de **brinde de boas-vindas**, concedido de graça na
  criação do pintor (não comprável com pontos). Fica **fora da grade normal** da lojinha do pintor
  (a query do catálogo filtra `is_brinde = false` e `active = true`), mas o resgate ainda o
  referencia por `loja_item_id` (reusa nome/imagem/estoque). Os dois itens-brinde têm IDs fixos
  (`…c1` boné, `…c2` pincel) e `valor_base = 0` (campo morto — o custo real é grátis).
- `promo_desde` `timestamptz` nulável — quando o item **entrou** em promoção (`null` = fora de
  promo). Mantido pelo trigger `trg_promo_desde` (`before insert or update of mult_delta`):
  carimba `now()` na **transição** pra `mult_delta < 0`, **preserva** em ajuste de promo já ativa
  (mudar o desconto não re-notifica), limpa ao sair. É a data de evento do card "Promoção na
  lojinha" do feed — sem ela o card usava `Date.now()` a cada render, sempre mais novo que
  `notif_visto_em`, e nunca marcava como lido.
- Custo em pontos é derivado (não há coluna de custo).

**`resgates`** — estado-máquina de 3 status.

- `pontos_congelados` `check (>= 0)` — snapshot do custo no momento do resgate, para a devolução
  reembolsar exatamente o que foi debitado (mesmo princípio do snapshot de itens de pedido).
  `0` = resgate **grátis** (brinde de boas-vindas): não gera linha no ledger e o cancelamento não
  lança `devolucao` (o check `valor <> 0` proíbe registrar 0).
...
### Sincronização (catálogo)

**`sync_control`** — cursor incremental da sync do Hiper, **um registro por recurso**.

- `recurso` (PK, ex.: `produtos`), `ponto_de_sincronizacao` (último `pontoDeSincronizacao`
  do Hiper), `atualizado_em`.
- **RLS ligada, sem policy** → ninguém autenticado lê/escreve; só o `service_role` (sync
  server-only, que bypassa RLS). Leitura admin fica para depois (policy aditiva) se a UI
  precisar exibir "última sincronização".
...
| `sync_control`                             | RLS sem policy: só o `service_role` (sync server-only) lê/escreve; nenhuma role autenticada acessa                    |
| `push_subscriptions`                       | pintor lê/gerencia **só as próprias** (self por `auth_user_id`, nas 4 operações); envio pelo admin via `service_role` |
...
Publicação `supabase_realtime` inclui `orders`, `point_transactions`, `resgates`, `loja_items` e
`settings`. `orders` e `resgates` têm `replica identity full` (recebem UPDATE de status → no
`postgres_changes` a RLS precisa do `painter_id` na linha pra filtrar por assinante em
UPDATE/DELETE); as demais ficam no `replica identity default` (PK): `point_transactions` é
append-only, e as policies de leitura de `loja_items`/`settings` não filtram por linha — a decisão
da RLS nunca depende da linha antiga. Consumido por um listener client (`RealtimeRefresh` no
layout de cada app) que só dispara `router.refresh()` — não lê o payload. `settings` na publicação
existe porque os preços da lojinha do pintor derivam do multiplicador global via view: mudar o
global reprecifica tudo **sem** gerar evento em `loja_items`.
...
só a `authenticated` — exceto `conceder_brinde_boas_vindas`, restrita a `service_role` (roda no
backend admin, sem JWT de usuário).
...
| `cancelar_resgate(resgate)`                  | pintor           | só o próprio e só `pendente_retirada`; marca `cancelado`; devolve `quantidade` de estoque; credita `devolucao` (+snapshot total) **apenas se `pontos_congelados > 0`** (brinde grátis não lança ledger)                                                                                                                                                                                                                                                                                                                                                           |
...
| `cancelar_resgate_admin(resgate)`            | admin            | `pendente_retirada→cancelado`; devolve `quantidade` de estoque + credita `devolucao` (snapshot) **se `> 0`**; espelha `cancelar_resgate`, mas gated por `is_admin` e resolvendo o pintor da própria linha                                                                                                                                                                                                                                                                                                                                                         |
...
| `salvar_notif_prefs(prefs)`                  | pintor           | upsert das preferências de notificação do próprio pintor em `painter_settings` (identidade pelo JWT); consumidas pelo feed de notificações (T6a)                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `marcar_notif_visto()`                       | pintor           | upsert de `painter_settings.notif_visto_em = now()` do próprio pintor (identidade pelo JWT); chamado ao abrir a tela de notificações. Carimba o marco de "visto até agora" que apaga a bolinha do sininho. Mesmo padrão do `marcar_brinde_visto`                                                                                                                                                                                                                                                                                                                  |
| `conceder_brinde_boas_vindas(painter)`       | **service_role** | concede o brinde de boas-vindas: **idempotente** (se o pintor já tem resgate de brinde, retorna o existente); `FOR UPDATE` no boné; se ainda há boné (`stock > 0`) **sorteia 50/50** (`random()`) entre boné e pincel — saiu boné, baixa 1 do estoque; esgotou o boné, todos recebem pincel; cria `resgates` grátis (`pontos_congelados = 0`, `iniciado_por = admin`); **sem ledger**. Chamada pelo `POST /api/pintores` após criar o pintor. **Sem `current_painter_id`** — roda em contexto service (sem JWT de usuário); o gate de admin fica no route handler |
| `marcar_brinde_visto()`                      | pintor           | upsert de `painter_settings.brinde_visto_em = now()` do próprio pintor (identidade pelo JWT); chamado ao fechar o modal de brinde. Padrão do `salvar_notif_prefs`                                                                                                                                                                                                                                                                                                                                                                                                 |
...
1. **Imagens via Supabase Storage — FEITO.** Bucket público `imagens` (webp-only, 5MiB). Item da
   ...
   `storage.objects` (leitura pública; escrita só service_role). Troca/remoção de foto apaga o
   arquivo antigo do bucket (`apagarImagemPorUrl`, best-effort, guard por marker). Resíduo mínimo:
   upload novo que falhe no save seguinte fica órfão (raro, inofensivo).
...
| `…_products_public_view.sql`            | view `products_public` (catálogo ao pintor, `security_invoker` off)                                                                                                                                                                                                                                                       |
...
| `…_cancelar_orcamento_rpc.sql`          | RPC `cancelar_orcamento(numero)` (pintor): só **pendente** e só o próprio (JWT + `FOR UPDATE` contra corrida com a aprovação do admin); pendente não tem ledger → nada a estornar                                                                                                                                         |
...
| `…_loja_item_imagem_pos.sql`            | `loja_items.imagem_pos_x/y` (smallint 0-100, null=centro) p/ recorte da imagem; expostos na view `loja_items_admin`                                                                                                                                                                                                       |
| `…_brinde_boas_vindas.sql`              | brinde de boas-vindas: `loja_items.is_brinde` + `stock` nulável; `resgates.pontos_congelados >= 0`; RPC `conceder_brinde_boas_vindas` (service_role) + `marcar_brinde_visto` (pintor); cancelamentos não lançam ledger p/ resgate grátis; `painter_settings.brinde_visto_em`; view `loja_items_admin` expõe `is_brinde`   |
| `…_notif_visto.sql`                     | notificações (não-lido): `painter_settings.notif_visto_em` + RPC `marcar_notif_visto` (pintor). O feed em si é derivado no front (sem tabela de notificações)                                                                                                                                                             |
| `…_resgate_cancelado_por.sql`           | `resgates.cancelado_por` (enum `resgate_origin`, nulável): **quem** cancelou (pintor × loja). Base da notificação "Resgate cancelado pela loja" — auto-cancelamento do pintor é silencioso. Os 2 RPCs de cancelar gravam; view `resgates_admin` expõe                                                                     |
| `…_realtime_loja_items.sql`             | `loja_items` entra na publicação Realtime (preço/promo editado no admin chega ao pintor sem reabrir o app); sem `replica identity full` — a policy de leitura não filtra por linha                                                                                                                                        |
| `…_push_subscriptions.sql`              | Web Push (T6c): tabela `push_subscriptions` (`endpoint` PK, `painter_id` cascade, `p256dh`/`auth`), índice por pintor, RLS self nas 4 operações; envio via `service_role`                                                                                                                                                 |
| `…_brinde_sorteio.sql`                  | `conceder_brinde_boas_vindas` vira sorteio de verdade: `random()` 50/50 boné/pincel enquanto há boné em estoque (antes era fila — boné determinístico até esgotar). Só o `if` muda; idempotência, `FOR UPDATE` e grants intactos; não retroativo                                                                          |
| `…_realtime_settings.sql`               | `settings` entra na publicação Realtime: `bonus_percent`/`multiplicador_padrao` refletem sem reload nos dois apps — os preços do pintor derivam do global via view, então mudar o global não gerava evento em `loja_items`. Sem `replica identity full` (mesmo racional de loja_items)                                    |
| `…_promo_desde.sql`                     | `loja_items.promo_desde` + trigger `trg_promo_desde` (carimba na **transição** pra `mult_delta < 0`, preserva em ajuste de promo ativa, limpa ao sair) + backfill das promos vigentes; view `loja_items_admin` expõe no fim. Data de evento do card de promo do feed — antes usava `Date.now()` e nunca marcava como lido |
